...
const generateImageLocation = (saveImageLocation, folderName) => {
...
const createImageSaveLocation = async (saveImageLocation, testFolderName) => {
  await fs.mkdirAsync(generateImageLocation(saveImageLocation, testFolderName));
}
...
const runImageComparison = async (imageA, imageB, test) => {
  try {
    ...
    console.log(e);
    throw new Error(`Failed, could not take screenshot, ensure that the base image exists`);
  }
};
...
const prepareEmptyFolderStructure = async (saveImageLocation, folderName) => {
  const fileExists = fs.existsSync(generateImageLocation(saveImageLocation, folderName));
  if(!fileExists) return createImageSaveLocation(saveImageLocation, folderName);

  const imageLocation = generateImageLocation(saveImageLocation, folderName);
...
const prepareTestFolderStructure = async (saveImageLocation) => {
  ...
  return generateImageLocation(saveImageLocation, testFolderName);
}
...
const runAll = async () => {
  ...
  const baseImageLocation = generateImageLocation(saveImageLocation, 'base');
